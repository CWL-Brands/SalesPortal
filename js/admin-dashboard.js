 ...
 * 
 * CLEANED VERSION - Removed duplicates and orphaned code
 */
...
class AdminDashboard {
    constructor(options = {}) {
        ...
        // GitConnector for data persistence
        this.gitConnector = null;
        
        // Admin Authentication
        this.adminEmails = [];
        this.defaultPassword = 'K@nva2025';
        this.floatingButton = null;
        this.loginModal = null;
        this.adminModal = null;
        this.isInitialized = false;
...
async saveProductField(productId, field, value) {
    ...
    /**
     * Save data to Git repository (GitHub Pages compatible)
     * UPDATED METHOD - Replace the existing saveDataToGit method with this
     */
        ...
        try {
            ...
            if (isGitHubPages) {
                ...
                if (this.gitConnector?.token) {
                    ...
                    console.warn('⚠️ GitConnector not available, falling back to localStorage');
                    return await this.saveToLocalStorageOnly(filename, data);
                }
            }
        } catch (error) {
    ...
    /**
     * Save directly to GitHub API (for GitHub Pages)
     * ADD THIS NEW METHOD after the saveDataToGit method
     */
    async saveToGitHubAPI(filename, data, githubConnection) {
        try {
            ...
            const branch = githubConnection.branch || 'main';
            ...
            const getUrl = `https://api.github.com/repos/${owner}/${repo}/contents/data/${filename}`;
            let sha = null;
            ...
            // Prepare commit
            const commitData = {
                message: `Update ${filename} via Admin Dashboard - ${new Date().toISOString()}`,
            ...
            return {
                success: true,
                message: 'Data saved to GitHub repository',
    ...
    /**
     * Save to localStorage only (fallback method)
     * ADD THIS NEW METHOD after the saveToGitHubAPI method
     */
    async saveToLocalStorageOnly(filename, data) {
        try {
            const content = typeof data === 'string' ? data : JSON.stringify(data, null, 2);
            ...
            console.log(`✅ ${filename} saved to localStorage`);
            
            // Show user instructions for manual GitHub sync
            this.showManualSyncInstructions(filename, content);
    ...
    /**
 * Show manual sync instructions to user
 */
showManualSyncInstructions(filename, content) {
    // Only show if we're on GitHub Pages and don't have a token
    const isGitHubPages = window.location.hostname.includes('github.io');
    const hasToken = this.connectionData.github && this.connectionData.github.token;
    
    if (isGitHubPages && !hasToken) {
        const instructions = `🔑 GitHub Token Required
        
To enable automatic Git saves, please:
1. Go to Settings > GitHub Integration
2. Add your GitHub Personal Access Token
3. Configure repository details

For now, your changes have been saved to localStorage.
You can manually copy this data to your repository:`;

        // Show notification with copy button
        this.showNotification(instructions, 'warning', 10000, {
            label: 'Copy Data',
            onClick: () => {
